...
module.exports = {
        ...
        try {
            ...
            if (validate.length > 0) {
                return res.status(400).json(
                    response(400, "validate Error", validate)
                );
            }

            if (!req.file) {
                return res.status(400).json(
                    response(400, "image must be uploaded")
                );
            }
            ...
            return res.status(201).json(
                response(201, "created", formattedItem)
            );
        } catch (error) {
    ...
    getItem: async (req, res) => {
        try {
            ...
            const formattedItems = item.map((i) => ({
                ...i.toJSON(),
                imageUrl: `${req.protocol}://${req.get("host")}/uploads/${i.image}`
            }));

            return res.status(200).json(
                response(200, "success", formattedItems)
            );
        } catch (error) {
            return res.status(500).json(
                response(500, "server Error", error.message)
    ...
    detailItem: async (req, res) => {
        try {
            ...
            const formattedItem = {
                ...detail.toJSON(),
                imageUrl: `${req.protocol}://${req.get("host")}/uploads/${detail.image}`
            };

            return res.status(200).json(
                response(200, "success", formattedItem)
            );
        } catch (error) {
            return res.status(500).json(
                response(500, "server Error", error.message)
    ...
    updateItem: async (req, res) => {
        try {
            ...
            const formattedItem = {
                ...newItem.toJSON(),
                imageUrl: `${req.protocol}://${req.get("host")}/uploads/${newItem.image}`
            };

            return res.status(200).json(
                response(200, "update", formattedItem)
            );
        } catch (error) {
            return res.status(500).json(
                response(500, "server Error", error.message)
